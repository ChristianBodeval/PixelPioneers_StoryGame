...
public class WormFSM : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        healthScript.Dead.AddListener(OnDead);
...
public class PatrolState : EnemyState
{
    public override void Enter(WormFSM enemy)
    {
        ...
        Vector3 randomPoint = HelperMethods.GetRandomPointInRadiusOnGround(enemy.patrolRadius, enemy.transform.position);
        
        //Make randomPoint closer to 2 distances closer to the player 
        randomPoint += (enemy.transform.position - randomPoint).normalized * 3f;
        enemy.aStarPathing.SetDirection(randomPoint);

        //Old method, just pathing
...
public class DiggingState : EnemyState
{
    ...
    private IEnumerator diggingCastingCoroutine;
    ...
    public override void Enter(WormFSM enemy)
    {
        ...
        diggingCastingCoroutine = enemy.InvokeAfterDelay(() =>
        {
            Exit(enemy);
            isCasting = false;
        }, enemy.diggingTime);
        ...
        if (!isOnCooldown)
        {
            ...
            /*//if currentPlayerPosition is not on the ground layer, then set the currentPlayerPosition to the player position
            if (!Physics2D.Raycast(currentPlayerPosition, Vector2.down, 0.1f, enemy.groundLayer))
            {
                currentPlayerPosition = enemy.player.transform.position;
            }*/
            ...
            StartCoroutine(diggingCastingCoroutine);
        }
        else
        {
            enemy.ChangeState(State.Chase);
    ...
    {
        currentPlayerPosition = enemy.player.transform.position;
        currentPlayerPosition += (Vector3)enemy.player.GetComponent<Rigidbody2D>().velocity.normalized;
        enemy.aStarPathing.SetDirection(currentPlayerPosition);
